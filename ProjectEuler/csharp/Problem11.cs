    ...
    public class Problem11 : IProblem
    {
        ...
        private long Vertical(int col)
        {
            ...
            for (int i = 0; i < 17; i++)
            {
                ...
                if (sum > verticalAnswer)
                    verticalAnswer = sum;
            }
            return verticalAnswer;
        }

        private long Horizontal(int row)
        {
            var horizontalAnswer = 1;
            for (int i = 0; i < 16; i++)
            {
                var sum = 1;
                for (int j = 0; j < 4; j++)
        ...
        private static int[][] DecodeData(string data)
        {
            ...
            var gridOfStrings = from line in listOfStrings
                                select line.Split(' ').Select(int.Parse).ToArray();

            return gridOfStrings.ToArray();
        }
        ...
        public Answer Solution()
        {
            ...
            for (int i = 0; i < 20; i++)
            {
                horiz = Math.Max(horiz, Horizontal(i));
                vert = Math.Max(vert, Vertical(i));
                leftToRightDown = Math.Max(leftToRightDown, LeftToRightDown(i));
                leftToRightUp = Math.Max(leftToRightUp, LeftToRightUp(i));
            }
            return new Answer
            {
                Description = Math.Max(horiz, Math.Max(vert, Math.Max(leftToRightUp, leftToRightDown))).ToString()
            };
        }
    }
}
